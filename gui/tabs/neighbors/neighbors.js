...
export default {
	...
	events: {
		...
		days: refresh,
		showid: refresh,
		formula, gifts, summary
	}
};

let tab, container, inputs, smartTable, palRows, palGifts;
...
function init() {
	...
	htm = String(htm).replace('@DAYS@', '<br><select name="days"></select>');
	const label = inputs.days.parentNode;
	Html.set(label, htm);
	label.querySelector('select').replaceWith(inputs.days);
	Html.set(inputs.days, getSelectDays(0));
...
function getSelectDays(selectedValue) {
	let htm = '';
	for (let days = 7; days <= 50; days++) htm += Html`<option value="${days}"${days == selectedValue ? ' selected' : ''}>${Locale.formatNumber(days)}</option>`;
	return htm;
}
...
function getState() {
	const state = {
		...
		region: inputs.region.options.length ? inputs.region.value : inputs.region.getAttribute('data-value'),
		search: inputs.search.value,
		gift: filterGifts,
		exp: filterExp || undefined,
		id: inputs.showid.checked && bgp.Data.isAdmin,
...
function setState(state) {
	...
	state.days = gui.setSelectState(inputs.days, state.days, 21);
	...
	inputs.showid.checked = state.id && bgp.Data.isAdmin;
...
function updateButton() {
	inputs.formula.classList.toggle('activated', getFilterExpression() != '');
	inputs.gifts.classList.toggle('activated', filterGifts != '');
}
...
function gifts() {
	...
	let htm = '';
	const info = Html.raw(String(Html.br(gui.getMessage('neighbors_advancedfilterinfo'))).replace('@DAYS@', `<select name="days">${getSelectDays(state.days)}</select>`));
...
function refreshDelayed() {
	scheduledRefresh = 0;
	const state = getState();
	Html.set(inputs.show.querySelector('option[value="nogift"]'), Html(gui.getMessage('neighbors_nogift', Locale.formatNumber(+state.days))));
