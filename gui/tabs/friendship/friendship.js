...
function showStats() {
    ...
    if (numToAnalyze == numAnalyzed || numToAnalyze == 0) {
        ...
        if (bgp.Data.friendsCollectDate > 0) {
            htm += HtmlBr `${gui.getMessage('friendship_friendupdateinfo',  Locale.formatDateTimeFull(bgp.Data.friendsCollectDate))}`;
        }
    } else {
    ...
    htm = HtmlBr `${gui.getMessage('friendship_totalfriends', Locale.formatNumber(numFriendsShown), Locale.formatNumber(numFriends))}`;
    for (let div of container.querySelectorAll('.numfriends')) div.innerHTML = htm;
    htm = HtmlBr `${gui.getMessage('friendship_totalneighbours', Locale.formatNumber(numNeighboursShown), Locale.formatNumber(numNeighbours))}`;
...
function updateRow(row) {
    ...
    if (friend) {
        ...
        if (pal) {
            ...
            htm += HtmlBr `<td></td><td>${friend.score == -1 ? buttonRegard : buttonIgnore}</td>`;
        }
    } else {
    ...
    if (pal) {
        ...
        htm += HtmlBr `<td></td><td></td><td></td><td></td>`;
    }
    row.innerHTML = htm;
